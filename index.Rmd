...
```
We can see here that the accuracy of combined classifiers is `r confusionMatrix(finalVote, testing_set$classe)$overall[1] * 100` percent. This is the out of sample accuracy (remember that accuracy could be a method to calculate model performance on testing and validation data set). We are going to use the same voting system to predict the labels in the validation data set.

...
```
We can see here that the accuracy of combined classifiers is `r confusionMatrix(finalVote, testing_set$classe)$overall[1] * 100` percent. This is the out of sample accuracy. It's slightly worse than the random forest model, but better than the other three. Therefore, it is still useful to pick this approach of predicting. We are going to use the same voting system to predict the labels in the validation data set.
